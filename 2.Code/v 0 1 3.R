...
fn_downloadZip <- function(URL) {
  ...
  # from "201307-citibike-tripdata.csv" to "2013-07 - Citi Bike trip data.csv"
  c.csv.file.ext = paste0(substr(c.csv.file,1,4),"-",substr(c.csv.file,5,6)," ","-"," ","Citi Bike trip data.csv")
  ...
  {
    cat("corresponding csv file: ", c.csv.file)
    cat("\n")
    message('file already exists')
...
# The closest I could get was SO
# SRC: 
# http://stackoverflow.com/questions/32103639/read-csv-file-in-r-with-double-quotes)
...
# 
# 
# c.temp.file = "reduced trip data.csv"
# c.temp.file = file.path(c.home.dir,c.data.dir,c.temp.file)
# 
# s<-file(c.temp.file,open="rt")
