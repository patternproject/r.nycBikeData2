...
## 0 2 0: a) changing fn_download, as follows:
##        c.files %>% map(fn_downloadZip)
##        to use c.files (complete) instead of 
##        readuced set c.files[1:3]
...
## 0 2 1: a) further reducing the data read in to db i.e. 
##        Gender == Male AND
##        User Type == Annual Member
##        b) i.chunk_size = 10,000 (from 1,000)
##        Despite the above changes and few hours, my laptop is unable to handle 
##        the volume of data
...
fn_append_to_sqlite <- function(x, pos) {
  ...
  x <-
    ...
    filter(gender == 1 & usertype == "Subscriber")
